...
<body class="inter  w-full h-full ">
    ...
    <section id="orders" class="w-full h-auto bg-gray-950 flex flex-col place-content-center place-items-center gap-6 py-18">
        ...
        <div class="h-auto w-full flex justify-center items-center gap-5 flex-wrap ">
            ...
            <!-- Jeep Wrangler -->
            <?php $getVehicleList = getVehicles() ?>
            <?php if (!empty($getVehicleList)): ?>
                <?php foreach ($getVehicleList as $vehicle): ?>
                    <div class="h-100 w-80 bg-white p-2 gap-2 transform hover:scale-102 duration-400 ease-in-out shadow-2xl">
                        <div class="h-[60%] w-full overflow-hidden">
                            <img src="./admin/uploads/<?= htmlspecialchars($vehicle['image']); ?>" alt="<?= htmlspecialchars($vehicle['image']); ?>"
                        ...
                        <div class="h-[40%] w-full flex flex-col justify-between p-2">
                            <div class="flex flex-col">
                                ...
                                <h2 class="text-lg font-semibold text-gray-900"><?= htmlspecialchars($vehicle['name']); ?></h2>
                                <p class="text-sm text-gray-600"><?= htmlspecialchars($vehicle['category']); ?></p>
                                <p class="text-sm text-gray-600"><?= htmlspecialchars($vehicle['description']); ?></p>
                            </div>
                            <a
                                href="https://wa.me/255788835182?text=Hello%20I%20am%20interested%20in%20your%20cars.%20Can%20you%20share%20details%3F"
                                target="_blank" rel="noopener"
                                class="cursor-pointer mt-2 px-3 py-1 bg-[#f36428] text-white text-center text-sm rounded hover:bg-[#d9531e]">
                                order now
                            </a>
                        </div>
                    </div>
    ...
    <img src="images/2.jpg" alt="" class="fixed top-0 left-0 right-0 w-full h-full object-cover -z-10">
    ...
    <!-- contact us -->
    <div id="contacts" class="h-auto w-full px-4 sm:px-8 md:px-12 py-4 sm:py-8 md:py-12 pb-15 flex items-center justify-center  bg-[#ff6324]">
        <div
            ...
            <form action="./index.php?action=contact" method="post" class="grid grid-rows-[0.5fr_1fr_1fr_2fr_1fr]    gap-4 px-6 sm:px-12 py-2 sm:py-8 ">
                ...
                <input type="text" placeholder="Your Email" name="email"
                    class="w-full h-full p-4 bg-white focus:outline-2 outline-[#ff6324]/70 outline-offset-4">
                <textarea placeholder="comment" name="message" cols="20" rows="3"
                    class="w-full h-full p-4 bg-white focus:outline-2 outline-[#ff6324]/70 outline-offset-4"
                    id=""></textarea>
                <button type="submit"
                    class="cursor-pointer w-full h-full p-4 bg-gray-800 text-white font-semibold hover:bg-gray-900 ease-in-out duration-300">Send</button>
            </form>
        </div>
    </div>
    <!-- footer -->
